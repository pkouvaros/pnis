...
class DQNStrategy(AbstractStrategy):
    DEFAULT_TRAINING_EPISODES: int = 128

    DEFAULT_POLICY_TRAIN_FREQ: int = 1
    DEFAULT_TARGET_NET_UPDATE_FREQ: int = 4
    DEFAULT_REPLAY_BUFFER_SIZE: int = 65536
    ...
    DEFAULT_EPSILON_MIN: float = 0.0001
    DEFAULT_EPSILON_DECAY: float = 0.95
    ...
    def __init__(self,
        ...
        self.dense_layer_width = dense_layers_width
        self.dense_layer_depth = dense_layers_depth
    ...
    def remember(self, experiences: List[Experience]):
    ...
    def update_target_network(self):
        ...
        self.target_net.set_weights( # type: ignore
            self.policy_net.get_weights()) # type: ignore
    ...
    def replay(self):
        ...
        for i, ((_, action, next_state, final), reward) in enumerate(minibatch):
            ...
            if not final:
                action_target += self.discount_factor * \
                    np.amax(self.target_net.predict( # type: ignore
        ...
        history: History = self.policy_net.fit(states, targets, batch_size=len(minibatch), verbose=0) # type: ignore
        loss = history.history['loss'][0]

        print(f"\rLoss: {loss:.4f}, Epsilon: {self.epsilon:.4f}", flush=True)
