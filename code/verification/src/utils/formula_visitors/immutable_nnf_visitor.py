...
class FormulaVisitorNNF(FormulaVisitorI):
    ...
    def visitNegationFormula(self, element):
        ...
        if isinstance(subformula, GammaUniversalFormula):
            return GammaExistentialFormula(subformula.k, subformula.gamma, subformula.not_gamma, NegationFormula(subformula.left).acceptI(self))

        if isinstance(subformula, GammaExistentialFormula):
            return GammaUniversalFormula(subformula.k, subformula.gamma, subformula.not_gamma, NegationFormula(subformula.left).acceptI(self))
