...
in a variety of languages, including temporal-epistemic logic and alternating-time
...
Neural-symbOlic interpreted Systems (\panos{}), where the template for the agents is
not purely symbolic but 
\begin{inparaenum}[\it (i)]
\item comprises a perception mechanism that is implemented
via neural networks, 
\item it is coupled with a symbolic action mechanism. 
\end{inparaenum} This
neural-symbolic treatment of the agents follows the Neural Interpreted Systems
(NIS) model from~\cite{Akintunde+20b}.  Differently from
\panos{} however, NIS are
limited to standard non-parametric systems with a pre-defined number of agents.

A \panos{} consists of the descriptions of an agent template, from which an
unbounded number of concrete agents may be constructed, and of an environment in
which the agents operate. The agent template is defined as follows. 
...
\item $\lstates t = \prvs t \times \pers t$ is a nonempty (possibly infinite)
  set of template \emph{local states}.  Each local state is a pair
  $\tuple{\prv t, \per t}$ of a private state
  $\prv t \in \prvs t \subseteq \mathbb R^{m_{prv}}$ and a percept
  $\per t \in \pers t \subseteq \mathbb R^{m_{per}}$ that encodes a perception
  of the environment by the agent.

  \item $\init t \in \lstates t$ is the unique \emph{initial template state}.
  
  \item $\obs t : \lstates t \times \lstates e \rightarrow \pers t$ is an
  \emph{observation function} that maps pairs of template local states $\lstates t
  ...
  \subseteq \mathbb{R}^{m_e}$ (see below) to percepts $\pers t \subseteq
        \mathbb{R}^{m_{per}}$. We assume, without loss of
        generality to the parameterised verification results
        present in the Secton~\ref{sec:verification}, that the observation function is implemented via a neural
  network $f : \mathbb{R}^{m_{prv} + m_{per} + m_e} \rightarrow
        \mathbb{R}^{m_{per}}$.

  \item $\acts t$ is a nonempty and finite set of \emph{actions}.
  
  \item
    $\prot t : \lstates t \rightarrow 2^{\acts t} \setminus \set{\emptyset}$ is
    a \emph{local protocol function} that returns the set of  actions available at a given local
    state.
    % that selects which actions may be performed at a given local state.

  \item
    $\tr t: \lstates t \times \acts t \times 2^{\acts t} \times \acts e
    \rightarrow \prvs t$ is a \emph{local transition function} that determines
    the next private state for an agent instantiated from the template given
    its current local state, its action, the set of actions performed by all
    the agents and the action of the environment.
...
\begin{example}\em
  ...
  We consider the example of a guarding game, an instance of
    a collective risk dilemma \cite{MilinskiSKRM2008}. In
    this game there is a colony of agents that needs to be
    guarded. Guarding duty costs a guard some health~$G_r$
    ($G_r < 0$) and resting improves an agent's health by
    $R_r$ $(R_r > 0)$.  If no one is guarding, then all
    agents in the colony lose some health~$U_r$ $(U_r < 0)$.
    The maximum value of an agent's health is~$M_h$. If an
    agent does not have any health left, then it
    \emph{expires}.
%
  We formalise the agent template
  $\ag t = \tuple{\lstates{t}, \init{t}, \obs{t}, \acts{t}, \prot{t}, \tr{t} }$
  ...
  \begin{itemize}[$\bullet$]\itemsep 0cm
  \item
    $\lstates{t} = \{(h,\per{}) \mid h \in \{0,\dots,M_h\},
          \ \per{} \in \{G_p, R_p, E_p\}$, where $h$ is the
          health of the agent and percepts $G_p$, $R_p$ and
          $E_p$ indicate what the agent is willing to do
          (see below).
     %stand for \emph{G}uarding, \emph{R}esting and \emph{E}xpired, respectively.

  \item $\init{t} = (|G_r| + 1, G_p)$, i.e., at the start each agent has enough
    health to be able to guard at least once without expiring.
  ...
  \item The observation function is assumed to be given as a neural network. It
    ...
    is expired, $E_p$.
    % But for the sake of illustration, we define here the observation
    % function of a perfectly altruistic agent:
  ...
  \item The local transition function updates the health according to the
    ...
    $\begin{array}{@{}l@{\quad}l}\small
       \tr{t}((h,P), G_a, A) = \max(h + G_r,0) &\\% \text{ and } h > G_p\\
       % \tr{t}((h,P), G_a, A) = 0     &\text{ if }G_a\notin A \text{ and } h \leq G_p\\[2mm]
       \tr{t}((h,P), R_a, A) = \min(h + R_r, M_h) &\text{ if }G_a\in A\\
       \tr{t}((h,P), R_a, A) = \max(h + U_r,0) &\text{ if }G_a\notin A\\% \text{ and } h > U_p\\
       % \tr{t}((h,P), R_a, A) = 0     &\text{ if }G_a\notin A \text{ and } h \leq U_p\\[2mm]
...
The environment has a  similar description to an agent
template but it 
\begin{inparaenum}[\it (i)] \item 
does
not include a perception mechanism, \item may include an
infinite set of initial local states. \end{inparaenum}

\begin{definition}[Environment]
  An {\em environment} is a tuple
  $\ag e = \tuple{\lstates e, \inite, \acts e, \prot e, \tr e }$, where
  $\lstates e \subseteq \mathbb R^{m_e}$ is a nonempty % (possibly) infinite
  set of \emph{local states}, $\inite \subseteq \lstates e$
    is a nonempty set of \emph{initial local states}, $\acts e$ is a nonempty and finite set of
  \emph{actions},
  $\prot{e} \colon\lstates{e}\rightarrow 2^{\acts e} \setminus \set{\emptyset}$
  is a \emph{local protocol function}, and
  $\tr e \colon \lstates e \times \acts e \times 2^{\acts t} \rightarrow
  \lstates e$ is a \emph{local transition function}.
\end{definition}

The agent template and the environment define the main
formal structure we will be using in this paper.

\begin{definition}%[Parameterised Neural Interpreted System]
    A \emph{Parameterised Neural Interpreted System
    (\panos{})} is a tuple $\mathcal S = \tuple{\ag t, \ag
    e, \ell}$, where $t$ is an agent
    template, $e$ is an environment, and $\tlabel{} :
    \atprop \rightarrow 2^{\lstates t}$ is a labelling
    function on the agent  template states for a set
...
\begin{example}
  ...
  Assume a dummy environment $e$ where $\lstates{e}= \inite
    = \{0\}$ and
  $\acts{e}=\{0\}$ are singleton states and actions, $\prot{e}(0) = \{0\}$ and
...
%
    Then $\mathcal{S} = (t,e,\ell)$ is a \panos{} for the guarding example.
\end{example}

A \panos{} $\mathcal S$ gives a parametric description of an unbounded collection
...
$\ag t$.
%
A \emph{global state}  in
$\sys n$ describes the system at a particular instant of
time. It is a tuple $q = \tuple{\lstate 1, \ldots, \lstate
n, \lstate e}$  of local states for all the agents and the environment in
$\sys n$.  For a
global state $q$, we write $\lprv{i}{q}$, $\lper{i}{q}$ and
$\ls{i}{q}$ to
denote the \emph{private part} $\prv i$ and the \emph{perception part} $\per i$
of the local state $\ls{i}{q} = \tuple{\prv i, \per i}$ of agent $\ag i$ in
$q$. The set $\globalstates n$ of all possible global states is
$\globalstates{n}=\lstates{1}\times\dots\times\lstates{n}\times
L_e$.
% is the Cartesian product of the agents' sets of local states.
% 
...
The set $\globalacts n$  of all
joint actions is $\globalacts{n} =
\acts{1}\times\dots\times\acts{n}\times\acts{e}$.
%% E
% is the Cartesian product of the agents' sets of local actions.
We now formally define the concrete NIS generated from
\panos{}.
% A concrete system's global states evolve over time in compliance with
% the following global transition function.
...
\begin{definition}[Concrete Neural Interpreted System]
  \label{def:concreteystem}
    Given a \panos{} $\mathcal S = \tuple{\ag t, \ag e, \ell}$ and $n \geq 1$, a
...
\ldots, \ag n, \ag e}, \globalinit n, \valuation n}$, where $\globalinit n =
    \set{\tuple{\init 1, \ldots, \init n, \init e} \mid
    \init e \in \inite}$ is the set of initial global states and
...
joint action, each agent $\ag i \in \agents n$ updates the private component of its
...
projection of the joint action for the agents  onto a set.  This
...
So, differently from the standard treatment of interpreted systems, the
transition function of a concrete agent in \panos{}  does not depend on the joint
...
a joint action, thereby reflecting the unbounded nature of
\panos{}. In other words,
...
\begin{definition}[Global transition function] 
  ...
  \item $\la{e}{\alpha} \in \prot{e}({\ls{e}{q}})$, $\tr e(\ls{e}{q},
      \la{e}{q}, \setprojection{\alpha}) = \ls{e}{q'}$; i.e., the environment’s action is
  protocol compliant and its local state is updated as per its local transition
  function and the actions that were performed in the round.
  ...
  as per its local transition function and the  actions
          performed at the round, and the perception part of its local state is updated as per
...
A \emph{path} in a model $\msys n$ is an infinite sequence
of global states and joint actions
$q^0\alpha^0q^1\alpha^1\dots$ s.t.\ $(q^i,\alpha^i,q^{i+1})
\in \globalrel n$ for all $i \geq 0$. For a path $\rho$, we
write $\rho(i)$ for the $i$-th global state in $\rho$. Given
a global state $q$ in $\msys n$ we write $\paths{q}$ for the
set of all paths originating from $q$.

% We now illustrate Definitions~\ref{def:concreteystem}-\ref{def:model} on our
% running example.

\begin{example}
  Consider a concrete neural interpreted system
    $\sys{2}=\tuple{\set{1,2,e}, \globalinit{2},
    \valuation{2}}$ for the agent template in
    Example~\ref{ex:agent-template} and \panos{} in
    Example~\ref{ex:pnis}, where $\valuation{2}$ is defined
    from $\ell$, $\globalinit{2} =
    \set{\tuple{\init{1},\init{2}, 0}}$ and $\init{i} =
    (|G_p|+1, G_p)$. A model $\msys{2}$ of $\sys{2}$ is
    depicted in Figure~\ref{fig:model-guarding}, where for
    brevity we omit the environment local state
    $\lstate{e}$.

  \begin{figure}
    \centering
    \begin{tikzpicture}[yscale=1.2, %
      gg/.style={green!50!red}, %
      ...
      \foreach \a/\hone/\perone/\htwo/\pertwo/\prop/\x/\y in { %
        s22gg/2/G_p/2/G_p/{a,a}/0/1.1,%
        s00/  0/E_p/0/E_p/{d,d}/1.1/0,%
        s11/  1/R_p/1/R_p/{a,a}/-1.1/0,%
        s31/  3/G_p/1/R_p/{a,a}/-3.3/0,%
        s13/  1/R_p/3/G_p/{a,a}/3.3/0,%
        s10/  1/R_p/0/E_p/{a,d}/-1.2/-0.9,%
        s01/  0/E_p/1/R_p/{d,a}/1.2/-0.9%
      }{ %
        \node[scale=0.7, outer sep=1mm,draw, very thick, rounded corners] (\a) at (\x,\y) {$(\hone,\perone), (\htwo,\pertwo)$};

        \node[fill=black, text=white, rounded corners, inner sep=1mm, scale=0.7, xshift=0.3mm, yshift=-1mm] at (\a.south) {$\mathsf{\prop}$};
      }

      \foreach \from/\to/\lab/\wh in { %
        s22gg/s00/{rr}/left, s22gg/s11/{gg}/left, s22gg/s31/{rg}/left, s22gg/s13/{gr}/right, %
        s31/s10/{rr}/left, %
        s13/s01/{rr}/left, %
        s10/s00/{re}/left,%
        s01.140/s00.210/{er}/right%
      ...
      \foreach \from/\to/\out/\in/\loose/\lab/\wh in { %
        ...
        s11.30/s00.150/30/150/1/{rr}/left%
      }{%
      ...
      \begin{scope}[yshift=1.2cm]
        ...
        }{ \draw[-latex, semithick, \col] (2.8,-0.18*\y) -- ++(0.8,0) node[right, scale=0.6] {$\lab$}; }
        \foreach \col/\y/\lab in { %
          re/0/{R_a,E_a}, er/1/{E_a,R_a}, ee/2/{E_a,E_a}%
        }{ \draw[-latex, semithick, \col] (-4.2,-0.18*\y) -- ++(0.8,0) node[right, scale=0.6] {$\lab$}; }
      \end{scope}
    \end{tikzpicture}
...
We express specifications for \panos{} in an indexed and bounded variant of
...
\item introduces indexed atomic propositions that are quantified over the
  agents of the concrete system on which the formula in
    question is evaluated, and
...
restricts $\ctl$ to bounded formulae~\cite{Akintunde+20}.
...
holds at the $k$-th step''. We note that bounded until
$A(\varphi U^k \psi)$ can be inductively abbreviated as
in~\cite{Akintunde+20}.
 %\begin{align*}
...
%analogously. 
% We inductively abbreviate bounded until as
% \begin{align*}
%  A(\varphi U^1 \psi) &\triangleq \psi \lor (\varphi \land AX^1 \psi) \\
%  A(\varphi U^k \psi) &\triangleq \psi \lor (\varphi \land AX^1 (A (\varphi U^{k-1} \psi)).
% \end{align*}
...
The logic $\bctl$~\cite{Akintunde+22} is the logic obtained
from the above BNF, but replacing the first clause with $p$
and removing the last clause. The restriction of the logic
to bounded properties follows the undecidability of
verification for the unbounded case~\cite{Akintunde+22}.
While the parameterised verification results presented in
the next section still hold should negation be allowed in
our syntax, verification methods for concrete models,
including \venmas used in the present contribution, do not
support negation, since its inclusion may hinder the
completeness of verification~\cite{Akintunde+22}.

A $\bictl$ formula is said to be a \emph{sentence} if every variable appearing
in the formula is in the scope of either a universal or an
existential agent 
quantifier. Hereafter we consider only indexed $\bictl$ sentences of the form
$\forall v_1 \cdots \forall v_m \colon \varphi$, or, shortly, $\bictlspec$,
where:
\begin{inparaenum}[\it (i)]
\item $v_i \in \atvar$ for $i \in \set{1,\ldots,m}$,
\item $\varphi$ is either {\em
universal} in that it contains only universal path quantifiers or {\em existential}
in that it contains only existential path quantifiers.
\end{inparaenum}
...
\begin{definition}[Satisfaction]
  \label{def:sat} 
  Given a model $\msys n$, a global state~$q$ in $\msys n$,
    and a $\bictl$ sentence $\bictlspec$ with $m \leq n$,
    the \emph{satisfaction} of $\varphi$ at $q$, denoted
    $(\msys n, q) \models\varphi$, or simply $q \models
    \varphi$ when $\msys n$ is clear from the context, is
    defined as:
  \begin{description}
  \item[$q \models (p, i)$] \ iff \ $q \in \valuation n((p,
      i))$, for $p \in
...
number of time steps.  Given a $\bictl$ sentence, we call this number of steps
the {\em temporal depth} of the sentence, which we formally define below.
...
A sentence $\bictlspec$ is said to \emph{be true} in $\msys
n$, denoted $\msys n \models \bictlspec$, if $(\msys n, q)
\models \bictlspec$ for all $q \in \globalinit n$. The
sentence is said to \emph{be true} in $\npis$, denoted
$\npis \models \bictlspec$, if it is true in every
model induced by every concrete system instantiated from
$\npis$ with at least $m$ agents, i.e., $\msys n \models
\bictlspec$ for all $n \geq m$.  The {\em parameterised
...
often called \emph{emergent properties}~\cite{Bonabeau99}, are
...
number of agents to be present for the underlying formations to be established. A
...
\begin{definition}[Emergence identification problem]
