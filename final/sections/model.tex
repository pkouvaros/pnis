...
systems put forward to reason about \textcolor{red}{unbounded} multi-agent
...
\item $\lstates t = \prvs t \times \pers t$ is a nonempty (possibly infinite)
  ...
  \subseteq \mathbb{R}^{m_e}$ (see below) to percepts $\pers t \subseteq
        \mathbb{R}^{m_{per}}$. \color{red}{We assume without loss of
        generality that the observation function is implemented via a neural
  ...
  \item
    ...
    a \emph{local protocol function} returning actions available at a given local
    state.
...
\begin{definition}[Agent environment]
  An {\em agent environment} is a tuple
  $\ag e = \tuple{\lstates e, \acts e, \prot e, \tr e }$, where
  $\lstates e \subseteq \mathbb R^{m_e}$ is a nonempty % (possibly) infinite
  set of \emph{local states}, $\acts e$ is a nonempty and finite set of
  \emph{actions},
...
\begin{definition}%[Parameterised Neural Interpreted System]
  ...
  = \tuple{\ag t, \ag e, \ell}$, where \textcolor{red}{$t$ is an agent template, $e$ is an agent environment, and} $\tlabel{} : \atprop \rightarrow
...
Given a PNIS $\mathcal S = \tuple{\ag t, \ag e, \ell}$ and $n \geq 1$, a
...
Differently from the standard treatment of interpreted systems, the
...
A \emph{path} in a model $\msys n$ is an infinite sequence of global states and joint
actions $q^0\alpha^0q^1\alpha^1\dots$ s.t.\ $(q^i,\alpha^i,q^{i+1}) \in
...
\begin{example}
  ...
  Example~\ref{ex:pnis}, where $\valuation{2}$ is defined from $\ell$,
  $\globalinit{2} = \tuple{\init{1},\init{2}, \init{e}}$ and
  $\init{i} = (|G_p|+1, G_p)$. For brevity we omit the environment local state
  $\lstate{e}$.  A model $\msys{2}$ of $\sys{2}$ is depicted in
  Figure~\ref{fig:model-guarding}.
...
holds at the $k$-th step''. \textcolor{red}{We note that bounded until
...
and removing the last clause. \textcolor{red}{The restriction of
the logic to bounded properties follows the undecidability
of verification for the unbounded case. While the
parameterised verification results presented in the next
section still hold should negation be allowed in our syntax,
verification methods for concrete models, including \venmas
used in the present contribution, do not support negation,
since its inclusion may hinder the completeness of
verification~\cite{Akintunde+22}.}

A $\bictl$ formula is said to be a \emph{sentence} if every variable appearing
in the formula is in the scope of either a universal or an existential
...
universal} in that it contains only universal quantifiers or {\em existential}
in that it contains only existential quantifiers.
\end{inparaenum}
...
\begin{definition}[Satisfaction]
  ...
  \begin{description}
  \item[$q \models (p, a)$] \ iff \ $q \in \valuation n((p,
      a))$, for $p \in
