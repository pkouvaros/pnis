...
agents were learning to play against exact copies of themselves. \textcolor{red}{Probably here we should see more details about the simulations. How it was chosen who guards. What the rewards are}
...
%
Specifically, minibatches consisted of predictions made by the \emph{policy
  network} for the Q-values of each action in the \emph{starting state} of each
experience in the minibatch.

\nb{E: I don't understand this part. }%
Then, predictions are made for the Q-value of the \emph{most valuable} action
given the next state of each experience, and this is multiplied by the discount
factor and then added to the prediction previously made by the \emph{policy
  network} that corresponds to the estimated value of the action that was taken
in that transition.
%
