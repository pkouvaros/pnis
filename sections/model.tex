...
actions $q^0\alpha^0q^1\alpha^1\ldots$ such that $(q^i,\alpha^i,q^{i+1}) \in
...
\begin{example}
  ...
  $\init{i} = (-G_p+1, G_s)$. Assume that $G_p=-1$, $R_r=1$, $U_p=-2$ and
  $M_h \geq 4$. For brevity we omit the environment local state $\lstate{e}$.  The
  model $\msys{2}$ of $\sys{2}$ can be depicted as follows.
  ...
  \begin{center}
    ...
    \foreach \a/\hone/\perone/\htwo/\pertwo/\prop/\x/\y in { %
      ...
      \begin{scope}[yshift=1.4cm]
        ...
        }{ \draw[-latex, semithick, \col] (2.6,-0.18*\y) -- ++(1,0) node[right, scale=0.6] {$\lab$}; }
        \foreach \col/\y/\lab in { %
          re/0/{R_a,E_a}, er/1/{E_a,R_a}, ee/2/{E_a,E_a}%
        }{ \draw[-latex, semithick, \col] (-4,-0.18*\y) -- ++(1,0) node[right, scale=0.6] {$\lab$}; }
    \end{scope}
    \end{tikzpicture}
  \end{center}
