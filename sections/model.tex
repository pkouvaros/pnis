...
$\pprv a = \tr a ( \lstate a, \act a, A, \act e)$, where $A = \set{ \act b \mid
b  \in \set{1, \ldots, n}}$.  This generates an intermediate local state $\plstate{a} =
\tuple{\pprv a, \per a}$ for each of the agents. Similarly, the  environment
updates its local state to $\plstate e$. Finally, every agent $\ag a \in \agents
n$ observes the update on its local state and the update on the local state of
the environment via its neural perception module $\obs a$, thus generating a
percept $\pper a = \obs a(\plstate a, \plstate e)$, with which it updates its
perception part thereby obtaining a new local state $\pplstate a = \tuple{ \pprv
a, \pper{a}}$.
...
\begin{example}
  ...
  \begin{array}{l@{~}l}
    ...
    % \tr{t}((h,R_s), G_a, A) = & (h - G_p, G_s) \text{ if }G_a\notin A \text{ and } h > G_p\\
    % \tr{t}((h,R_s), G_a, A) = & (0, E_s) \text{ if }G_a\notin A \text{ and } h \leq G_p\\
    % \tr{t}((h,R_s), R_a, A) = & (h + R_r, R_s) \text{ if }G_a\in A\\
    % \tr{t}((h,R_s), R_a, A) = & (h - U_p, R_s) \text{ if }G_a\notin A \text{ and } h>U_p\\
    % \tr{t}((h,R_s), R_a, A) = & (0, E_s) \text{ if }G_a\notin A \text{ and } h \leq U_p\\[2mm]
...
\begin{example}
  ...
  $\sys{2}=\tuple{\set{1,2,e}, \globalinit{2}, \valuation{2}}$ for the
  agent template in Example~\ref{ex:agent-template}, where
  $\globalinit{2} = \tuple{\init{1},\init{2}}$ and $\init{i} = (2, R_s)$.

  A model of $\sys{2}$ can be depicted as follows.

  \begin{tikzpicture}

    \foreach \a/\lab/\x/\y in { %
      s22r/{(2,R_s),\ (2,R_s)}/0/1,%
      s00/{(0,E_s),\ (0,E_s)}/-3/0,%
      s31/{(3,R_s),\ (1,G_s)}/0/0,%
      s13/{(1,G_s),\ (3,R_s)}/3/0,%
      s10/{(1,R_s),\ (0,E_s)}/-3/-1,%
      s22gr/{(2,G_s),\ (2,R_s)}/0/-1,%
      s40/{(4,R_s),\ (0,E_s)}/3/-1%
    }{ %
      \node[scale=0.8] (\a) at (\x,\y) {$\lab$};
      }

      \foreach \from/\to/\lab/\wh in { %
        s22r/s00/{R_a,R_a}/left, s22r/s31/{G_a,R_a}/left, s22r/s13/{R_a,G_a}/right, %
        s31/s10/{R_a,R_a}/left, s31/s22gr/{R_a,G_a}/left, s31/s40/{G_a,R_a}/right%
      }{ \draw[-latex] (\from) -- node[\wh, scale=0.7] {$\lab$} (\to);
      }
    \end{tikzpicture}
\end{example}
